...
/* merge: keep existing values, overwrite changed ones */
$newData = array_merge($oldData, $array);

$DBLIB->where("instances_id", $AUTH->data['instance']["instances_id"]);
$result = $DBLIB->update("instances", [
    "instances_calendarConfig" => json_encode($newData)
]);

if (!$result) {
    finish(false, ["code" => "UPDATE-FAIL", "message" => "Could not update instance calendar config"]);
} else {
    $bCMS->auditLog(
        "EDIT-INSTANCE",
        "instances",
        "Calendar - " . json_encode($newData),
